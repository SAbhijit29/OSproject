...
int get_next(int index){
	for(int i=0;i<q_proc;i++){
		if(proc[i].at<=clock && proc[i].bt!=0){
			proc[i].my_index=i;
			if(proc[i].p>=cur_p){
...
main(){
	...
	for(int i=0;i<6;i++){
		...
		printf("\nThe process %d has turn around time : %d",proc[i].id,proc[i].waiting_time);
	}
	
	for(int i=0;i<6;i++){
		proc[i].turn_around_time = proc[i].waiting_time + proc[i].bt_copy;
		printf("\nThe process %d has waiting time : %d",proc[i].id,proc[i].turn_around_time);
	}
}
